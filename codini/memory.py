...
session history 负责保存完整事件流
这个模块只保存更小的一层工作集：当前任务摘要、最近接触的文件、文件短摘要，以及少量跨轮笔记。
这样下一轮 prompt 还能接上上一轮，但不会被整段历史塞满。

Working Memory：每轮任务的临时状态（任务摘要、读过的文件）
Episodic Notes：情景笔记，会话结束后可能被丢弃
Durable Memory：写到磁盘的长期知识，按"主题（Topic）"归类，重启后依然存在
...
def reslove_workspace_path(raw_path, workspace_root = None):
    """
    解析并验证路径是否在工作区内
    返回: 绝对路径对象或 None（如果越界）
    """
    ...
    try:
        # 尝试获取从 root 到 resolved 的相对路径
        resolved.relative_to(root)
    except ValueError:
        # 不是父子关系 → 路径越界（不安全）
        return None
    # 如果没有异常 说明 resolved 是 root 的子路径
    return resolved
...
def canonicalize_path(raw_path, workspace_root=None):
    ...
    返回: 相对路径字符串（POSIX格式）
    """
    resloved = reslove_workspace_path(raw_path, workspace_root)
    if resloved is None:
        return Path(str(raw_path)).as_posix()
    if workspace_root is None:
        return Path(str(raw_path)).as_posix()
    root = Path(workspace_root).resolve()
    return resloved.relative_to(root).as_posix()

def file_freshness(raw_path, workspace_root=None):
    resolved = reslove_workspace_path(raw_path, workspace_root)
...
def normalize_memory_state(state, workspace_root=None):
    ...
    # 第一步 规范化 working memory（任务摘要 + 最近文件）
...
def retrieval_candidates(state, query, limit=3, workspace_root=None):
    ...
    if workspace_root is not None:
        durable_root = Path(workspace_root) / ".codini" / "memory"
...
class LayeredMemory:
    def __init__(self, state=None, workspace_root=None):
        ...
        self.durable_store = DurableMemoryStore(
            Path(workspace_root) / ".codini" / "memory") if workspace_root is not None else None
